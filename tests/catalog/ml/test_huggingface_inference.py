"""Tests for the HuggingFace inference catalog."""

from __future__ import annotations

import pytest

from vnvspec.catalog.ml.huggingface_inference import (
    attention_masks,
    generation,
    structured_outputs,
    tokenization,
...
class TestHuggingFaceInferenceCatalog:
    def test_total_count(self) -> None:
        total = sum(len(fn()) for _, fn in _CATALOG_FUNCTIONS)
        assert total >= 24, f"Expected >= 24 requirements, got {total}"
    ...
    def test_all_ids_start_with_cat_hgf(self) -> None:
        for _, req in _ALL_REQS:
            assert req.id.startswith("CAT-HGF-"), f"{req.id} bad prefix"

    def test_compatible_with_declared(self) -> None:
        import vnvspec.catalog.ml.huggingface_inference as pkg

        assert "transformers" in pkg.__compatible_with__
    ...
    def test_submodule_has_compat_pin(self, name: str, fn: object) -> None:
        ...
        mod = importlib.import_module(f"vnvspec.catalog.ml.huggingface_inference.{name}")
