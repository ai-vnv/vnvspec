"""Tests for the SQLAlchemy catalog."""

from __future__ import annotations

import pytest

from vnvspec.catalog.web.sqlalchemy import schema, session, transactions
...
class TestSQLAlchemyCatalog:
    ...
    def test_all_ids_start_with_cat_sqa(self) -> None:
        for _, req in _ALL_REQS:
            assert req.id.startswith("CAT-SQA-"), f"{req.id} bad prefix"

    def test_compatible_with_declared(self) -> None:
        import vnvspec.catalog.web.sqlalchemy as pkg

        assert "sqlalchemy" in pkg.__compatible_with__
